...
class mdf{
	...
	public function __construct(){
		...
		$this->__oApps=new \mdf\app_router($this);
		$this->__oRequest=new \mdf\request($this);
		$this->__oOperations=new \mdf\operations($this);
		$this->__oResponse=new \mdf\response($this);
		\set_error_handler([$this,'__error']);
		\set_exception_handler([$this,'__exception']);
		$this->__aConfig=$this->apps->mdf->controllers->system->setup();
		if(isset($_GET['action'])&&$_GET['action']==='bootstrap'){
			$this->apps->mdf->controllers->system->bootstrap();
			$this->response->reply();
		}elseif(!$this->apps->mdf->controllers->system->auth()){
			$this->response->reply(401);
		}elseif(isset($_GET['action'])&&$_GET['action']==='login'){
			$this->apps->mdf->controllers->system->login();
			$this->response->reply();
		}elseif(isset($_GET['action'])&&$_GET['action']==='logout'){
			$this->apps->mdf->controllers->system->logout();
			$this->response->reply();
	...
	public function __autoload($class){
		if(\preg_match('#^mdf\\\\(\w+)$#i',$class,$matches)){
			include_once(__DIR__.'/'.$matches[1].'.php');
		}elseif(preg_match('#^mdf\\\\requests\\\\(\w+)$#i',$class,$matches)){
			include_once(__DIR__.'/requests/_requests.php');
			include_once(__DIR__.'/requests/'.$matches[1].'.php');
		}elseif(preg_match('#^mdf\\\\dbs\\\\(\w+)$#i',$class,$matches)){
