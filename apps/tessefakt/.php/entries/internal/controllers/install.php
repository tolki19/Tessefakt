...
foreach($aFiles as $sFile) $this->connectors->db->multi(file_get_contents($sFile));
	...
	public function create_data():void{
		...
		// }
		$aApps['tessefakt']=$this->app->entries->internal->controllers->app->create_app(
			[
				'key'=>'tessefakt',
		...
		$this->app->entries->internal->controllers->app_tables->create_tables(
			$aApps['tessefakt'],
		...
		$aGroups['admin']=$this->app->entries->internal->controllers->group->create(
			[
				'name'=>'Admins'
			]
		);
		$aUsers['florian']=$this->app->entries->internal->controllers->user->create(
			[
		...
		$this->app->entries->internal->controllers->user->groups->create(
			$aUsers['florian'],
			$aGroups['admin'],
			[]
		);
