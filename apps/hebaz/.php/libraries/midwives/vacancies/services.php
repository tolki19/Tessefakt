...
class services extends \tessefakt\library
{
	public function create(
		...
		return $this->_create(
			midwife_vacancy: $midwife_vacancy,
			service: $service,
	...
	protected function _create(
		...
		$this->connectors->db->query('
			insert into `midwives-vacancies-services`
			set
				`midwives-vacancy`=' . $midwife_vacancy . ',
				`service`=' . $service . ',
	...
	protected function _update(
		...
		$this->connectors->db->query('
			update `midwives-vacancies-services`
			set
				`midwives-vacancy`=' . $midwife_vacancy . ',
				`service`=' . $service . ',
				`public-remark`=' . (is_null($public_remark) ? 'null' : '"' . $this->connectors->db->escape($public_remark) . '"') . ',
	...
	protected function _delete(
		...
		$this->connectors->db->query('
			delete from `midwives-vacancies-services`
			where `id`=' . $id . '
		');
		return $id;
	}
