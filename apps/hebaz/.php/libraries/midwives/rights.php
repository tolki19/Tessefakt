...
class rights extends \tessefakt\library
{
	...
	protected function _create(
		...
		$this->connectors->db->query('
			insert into `midwives-rights`
			set
				`midwife`=' . $midwife . ',
				`_group`=' . (is_null($group) ? 'null' : '"' . $this->connectors->db->escape($group) . '"') . ',
				`_user`=' . (is_null($user) ? 'null' : '"' . $this->connectors->db->escape($user) . '"') . ',
