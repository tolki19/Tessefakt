...
class statics extends \tessefakt\library{
	...
	protected function _create(string $keystring,string $content,string|null $internal_caption,string|null $internal_remark):int{
		$this->connectors->db->query('
			insert into ``
			set
				`keystring`="'.$this->connectors->db->escape($keystring).'",
				`content`="'.$this->connectors->db->escape($content).'",
				`internal-remark`='.(is_null($internal_caption)?'null':'"'.$this->connectors->db->escape($internal_caption).'"').'
