	...
	_displayCycle(){
		...
		if(!this.bucket.data.sequence.length){
			...
			for(var i=0;i<this.bucket.data.sequence.length;++i){
				...
				if(this._oMdf.mscript({script:this._oConfig.index,water:oWater}).value==this._oMdf.mscript({script:this._oConfigkey,water:oWater}).value) this._displayChildren({water:oWater});
			}
		}
	}
