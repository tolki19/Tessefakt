	...
	constructor({tessefakt,parent,config,events}){
		...
		if(this._oConfig.post){
			for(var i=0;i<this._oConfig.post.length;++i){
				if(this._oConfig.post[i].field) this._oTessefakt.mscript({script:this._oConfig.post[i].field,water:this.water}).addEventListener('change',this._oEvents.change);
			}
		}
	}
	destructor(){
