var cTessefaktElementFormDisplay=class{
	...
	_display(){
		...
		this._oTessefakt.mscript({script:this._oConfig.index,water:this.water}).addEventListener('change',this.$change);
	}
	_displayCycle(){
		...
		if(!this.bucket.data.sequence.length){
			...
			for(var i=0;i<this.bucket.data.sequence.length;++i){
				...
				if(this._oTessefakt.mscript({script:this._oConfig.index,water:oWater}).value==this._oTessefakt.mscript({script:this._oConfig.key,water:oWater}).value) this._displayChildren({water:oWater});
			}
		}
	}
