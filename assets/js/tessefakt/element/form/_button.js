var cTessefaktFormButton=class{
	...
	constructor({tessefakt,parent,config,water}){
		...
		for(var i=0;i<(this._oConfig.gadgets??[]).length;++i){
			...
			try{
				this._oGadgets[this._oConfig.gadgets[i].type]=new window[sObject]({tessefakt:this._oTessefakt,parent:this,config:this._oConfig.gadgets[i],element:this._dElement});
			}catch(ex){
		...
		for(var i=0;i<(this._oConfig.contents??[]).length;++i){
			...
			this._aSubjects.push(new window[sObject]({tessefakt:this._oTessefakt,parent:this,config:this._oConfig.contents[i],water:this.water}));
		}
	}
	_display(){
